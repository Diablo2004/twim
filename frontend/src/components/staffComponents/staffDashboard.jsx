...
export default class StaffDashboard extends Component {
    render() {
        ...
        const isStaff = UserServiceApi.isUserStaff();
        return (
            <Container style={{ margin: "0", padding: "0", maxWidth: "100%" }}>
                <Row style={{ margin: "0" }}>
                    <Col className="sidenav" md={2}>
                        <h3>{isStaff ? "Staff" : "Admin"} Functions</h3>
                        <p><a href="/staff" style={(currentLocation === "/staff") ? FunctionSelectedStyle : FunctionNotSelectedStyle}>Overview</a></p>
                        {
                            (isAdmin || isStaff) &&
                            <>
                                {isAdmin && <p><a href="/admin/signup" style={(currentLocation === "/admin/signup") ? FunctionSelectedStyle : FunctionNotSelectedStyle}>Create Account</a></p>}
                                {isStaff && <p><a href="/admin/addcars" style={(currentLocation === "/admin/addcars") ? FunctionSelectedStyle : FunctionNotSelectedStyle}>Create Car</a></p>}
                                {isStaff && <p><a href="/admin/addlocation" style={(currentLocation === "/admin/addlocation") ? FunctionSelectedStyle : FunctionNotSelectedStyle}>Add Location</a></p>}
                                {isAdmin && <p><a href="/admin/view/customers" style={(currentLocation === "/admin/customers") ? FunctionSelectedStyle : FunctionNotSelectedStyle}>View All Customers</a></p>}
